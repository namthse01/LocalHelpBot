...
Model config đọc từ config.py (CHAT_MODEL, LARGE_MODEL, EMBED_MODEL).

Virtual models:
  cad-rag       → RAG ChromaDB + CHAT_MODEL
  ui-agent      → UI/Frontend specialist + CHAT_MODEL
  code-agent    → agentic loop: read/write files, run commands, fix bugs
  web-creep     → agentic loop: search web, fetch URLs, read content
  browser-agent → đọc Chrome/Edge cookies & localStorage
  deep-agent    → pass-through sang LARGE_MODEL (glm-4.7-flash)
...
from config import OLLAMA_BASE, CHAT_MODEL, LARGE_MODEL, PROXY_PORT
...
REAL_MODEL = CHAT_MODEL  # alias dùng trong file này
...
DEEP_MODEL    = "deep-agent"  # dùng glm-4.7-flash cho tác vụ cần suy luận sâu
AUTO_MODEL     = "auto-agent"  # Smart Router
...
def _route_query(query: str) -> str:
    routing_prompt = f"""Analyze the user query and return ONLY the name of the best agent to handle it.
    Agents:
    - cad-rag: AutoCAD/CAD technical questions, .dwg, .dxf, AutoCAD .NET API.
    - ui-agent: UI/Frontend, WinForms, WPF, XAML, React, Vue, CSS, HTML.
    - code-agent: Complex coding tasks, fixing bugs, reading/writing multiple files, running commands.
    - web-creep: Researching the web, fetching URLs, finding current information.
    - browser-agent: Reading local browser cookies, storage, or session data.
    - deep-agent: Deep reasoning, complex logic, architecture, high-level planning.
    ...
    try:
        status, resp_body, _ = _forward("/api/chat", json.dumps(payload).encode(), {})
        if status == 200:
            resp = json.loads(resp_body)
            choice = resp.get("message", {}).get("content", "").strip().lower()
            for m in [CAD_MODEL, UI_MODEL, CODE_MODEL, WEB_MODEL, BROWSER_MODEL, DEEP_MODEL]:
                if m in choice:
                    return m
    except Exception as e:
...
def _tags_with_virtual() -> bytes:
    ...
    virtual = [
        ...
        {
            "name": DEEP_MODEL, "model": DEEP_MODEL,
            "modified_at": "2026-01-01T00:00:00Z", "size": 0,
            "details": {"parameter_size": "19B", "family": "Deep reasoning agent (glm-4.7-flash)"}
        },
        {
            "name": AUTO_MODEL, "model": AUTO_MODEL,
            "modified_at": "2026-01-01T00:00:00Z", "size": 0,
            "details": {"parameter_size": "7.6B", "family": "Smart Router Agent"}
        },
    ]
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if model == AUTO_MODEL and is_chat:
            ...
            print(f"[router] routing query: {user_q!r}", flush=True)
            model = _route_query(user_q)
            print(f"[router] routed to: {model}", flush=True)
        ...
        # ── Deep agent — dùng glm-4.7-flash cho tác vụ suy luận sâu ──
