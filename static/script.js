...
async function loadConfig() {
    try {
        ...
        // Render with individual try-catch to prevent one failure from blocking others
        try { renderAgentSelect(); } catch(e) { console.error('Error rendering agent select:', e); }
        try { renderAgentsGrid(); } catch(e) { console.error('Error rendering agents grid:', e); }
        try { renderDiscordSettings(); } catch(e) { console.error('Error rendering discord settings:', e); }
        try { renderTasksList(); } catch(e) { console.error('Error rendering tasks list:', e); }
        try { renderModeSettings(); } catch(e) { console.error('Error rendering mode settings:', e); }
    } catch (error) {
...
async function sendMessage() {
    ...
    try {
        ...
        if (chatWindow && chatWindow.contains(loadingDiv)) {
...
function openAgentModal(id) {
    ...
    const title = $id('modal-agent-title');
    if (title) title.innerText = `Edit Specialist: ${id}`;

    const nameInp = $id('edit-agent-name');
    if (nameInp) nameInp.value = id;

    const promptInp = $id('edit-agent-prompt');
    if (promptInp) promptInp.value = profile.system_prompt || '';

    const modelInp = $id('edit-agent-model');
    if (modelInp) modelInp.value = profile.model || '';
...
async function saveAgentSettings() {
    ...
    if (await updateConfig({ agents: updatedAgents })) {
        closeAgentModal();
    }
}
...
function renderDiscordSettings() {
    ...
    const tokenInp = $id('cfg-discord-token');
    if (tokenInp) tokenInp.value = d.token || '';

    const serverInp = $id('cfg-discord-server');
    if (serverInp) serverInp.value = d.server_id || '';

    const chanInp = $id('cfg-discord-channels');
    if (chanInp) chanInp.value = (d.allowed_channels || []).join(', ');
}
...
async function saveDiscordSettings() {
    ...
    if (await updateConfig({ discord: updatedDiscord })) {
...
async function saveModeSettings() {
    ...
    const primary = {
        type: provider === 'local' ? 'local' : 'api',
        provider: provider !== 'local' ? provider : undefined,
        model: model,
        api_key: provider !== 'local' ? key : undefined,
    };
    ...
    if (await updateConfig({ providers: { primary, fallback } })) {
        alert('AI Mode updated successfully!');
    }
}
